...
int	MixedResolutionViewManager::AddView(bool external_framebuffer,int current_frame)
{
	if(views.size()>0)
	{
		// try to find a view that's old.
		int oldest_view_id=-1;
		int oldest_framenumber=-1;
		for(auto i:views)
		{
			int f=i.second->last_framenumber;
			if(oldest_framenumber<0||f<oldest_framenumber)
			{
				oldest_framenumber=f;
				oldest_view_id=i.first;
			}
		}
		if(oldest_view_id>=0&&current_frame-oldest_framenumber>1)
		{
