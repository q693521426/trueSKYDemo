...
vec4 calcDensity(Texture3D cloudDensity,vec3 texCoords,vec4 noiseval,vec3 fractalScale,float dist)
{
	...
	vec4 density		=sample_3d_lod(cloudDensity,cloudSamplerState,pos,dist*2.0);
	...
	for(int i=0;i<255;i++)
	{
		...
		if(fade>0)
		{
			...
			if(density.z>0)
			{
				...
				vec3 noise_texc			=world_pos.xyz*noise3DTexcoordScale/1.3+noise3DTexcoordOffset;
				noiseval				=texture_3d_wrap_lod(noiseTexture3D,noise_texc,3.0*fadeDistance);
				float radiance			=(.3+noiseval.x)*(1.0-density.z)*100.0/(4.0*3.14159*radius_km*radius_km);
...
float GetRainAtOffsetKm(Texture2D rainMapTexture,vec3 cloudWorldOffsetKm,vec3 inverseScalesKm,vec3 world_pos_km,vec2 rainCentreKm,float rainRadiusKm,float rainEdgeKm)
{
	...
	return				rain_lookup*saturate((rainRadiusKm-length(world_pos_km.xy-rainCentreKm.xy))/rainEdgeKm)
...
void ColourStep(inout vec4 colour[NUM_CLOUD_INTERP],inout float meanFadeDistance,inout float brightness_factor
	...
	,float solidDist_nearFar[NUM_CLOUD_INTERP],bool noise,bool do_depth_mix,float distScale)
{
	density.z				*=cosine;
	density.z				*=cosine;
	density.z				*=saturate(distanceKm/0.24);
