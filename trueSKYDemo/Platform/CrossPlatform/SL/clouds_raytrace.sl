											...
											,vec3 cloudIrRadiance2)
{
	...
	solidDist_nearFar[2-1]	=	nfd.y;
	float n									=	nfd.x;
	float f									=	nfd.y;
	for(int l=1;l<NUM_CLOUD_INTERP-1;l++)
	{
		float interp			=1.0-float(l)/float(2-1);
		// This is z. So let:
		//						Z				=	n (f/d-1) / (f-n)
		// i.e.					Z(f-n)/n		=	f/d - 1
		//						1 + Z(f-n)/n	=	f/d
		...
		solidDist_nearFar[l]	//=f*n/ (n+interp*(f-n));
								=lerp(solidDist_nearFar[0],solidDist_nearFar[2-1],1.0-interp);
	...
	for(i=0;i<768;i++)
	{
		...
		fade *= smoothstep(0.f,1.f,distanceKm/10);
		//fade *=saturate(exp(distanceKm)-1.f);
		...
		if(fade>0)
		{
			...
			if(found)*/
			{
				...
				vec4 density			=calcDensity(cloudDensity,cloudTexCoords,noiseval,fractalScale,fadeDistance);
				density.z				*= fade;
				float rain_val			=0.0;
				if(do_rain_effect)
				{
					// The rain fall angle is used:
					rain_val			=rainEffect*fade*GetRainAtOffsetKm(rainMapTexture,cloudWorldOffsetKm,inverseScalesKm, world_pos, rainCentreKm.xy, rainRadiusKm,rainEdgeKm);
				}
				if(density.z+rain_val>0)
				{
					...
					if(rain>0)
					{
						rain_val	*=saturate(0.6+texture_wrap_lod(noiseTexture3D,rainNoiseInvScale*world_pos+vec3(0,0,rainVerticalTexcoord),0).w);
						density.z		=max(density.z,rain_val);
					}
					//density.xy		*=1.0+wo;
