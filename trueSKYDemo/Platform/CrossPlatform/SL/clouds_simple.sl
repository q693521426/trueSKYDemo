											...
											,int numSteps)
{
	...
	float stepMul=1.0+K/float(numSteps);
	bool found=false;
...

		{
			...
			if(found)
			{
				...
				if(do_rain_effect)
				{
					...
					float dm			=rainEffect*GetRainAtOffsetKm(rainMapTexture,cloudWorldOffsetKm,inverseScalesKm, world_pos, rainCentreKm.xy, rainRadiusKm,rainEdgeKm);
