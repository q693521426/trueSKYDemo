...
#define MP_ALLOC_OBJECT(T) (T*)MP_ALLOC(sizeof(T), __alignof(T))
...
inline MicroProfileLogEntry MicroProfileMakeLogPayload(const char* pData, uint64_t nSize)
{
	...
	P.LogEntry = 0;
	memcpy(P.message, pData, static_cast<size_t>(nSize));
	MicroProfileLogEntry LE = P.LogEntry;
	LE |= ((uint64_t)MP_LOG_PAYLOAD)<<uint64_t(62ll);
	return LE;
}
...
{
	return ((uint64_t)nGroupIndex << uint64_t(48ll)) | ((uint64_t)nGroupMask << uint64_t(16ll)) | nTimer;
}
					...
					for(uint32_t j = 0; j < 2; ++j)
					{
						...
						for(uint32_t k = nStart; k < nEnd; ++k)
						{
							...
							if(MP_LOG_ENTER == nType)
							{
								...
								MP_ASSERT(nTimer>=0);
								MP_ASSERT(nTimer < static_cast<int>(S.nTotalTimers));
